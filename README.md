# s21_decimal 
You will implement the s21_decimal.h library in the C programming language. This library should add the ability to work with the "decimal" type, which is not in the language standard. Nevertheless, this type is critically important.
...
## Problems during work and their solutions:

 * **Decimal overflow** -> A second big_decimal structure was created, twice as large
 * **Converting a floating point number to decimal** -> The sprintf function was used, through which we will invert the rounded exponent of the required length
 * **Problem when comparing when the scale is > 28, but the number fits in a regular decimal format, for example when multiplying two decimals** -> All comparisons are converted to big_decimal
 * **Problem when comparing -0 and +0 equality** -> Separate check for zero, without taking into account the last bits[3]
 * **Division** ->

 *The process of transition to a column can be described sequentially by sequential actions:*
 ```
a) We take as many high-order digits of the dividend as necessary in order to consider the number greater than or equal to the divisor.
b) We determine the first digit of the quotient by choosing so that the product of the divisor and this digit are as close as possible to the current result, but do not exceed it.
c) Subtract the resulting product of the current number.
d) Transfer the next digit of the dividend to the remainder.
e) Repeat steps b-d until you have used all the divisible digits.
```

...
Реализация библиотеки s21_decimal.h на языке программирования С. Эта библиотека должна добавить возможность работы с типом «decimal», который отсутствует в стандарте языка. 
...
## Проблемы в ходе работы и их решения:

  * **Переполнение decimal** -> Была создана вторая структура big_decimal, в два раза больше
  * **Преобразование float to decimal** -> Была использована функция sprintf через которую мы получали округленную экспоненту нужной длинны
  * **Проблема при сравнение когда scale** > 28, но число умещается в обычный decimal, например при умножение двух decimal -> Все сравнения переведены на big_decimal
  * **Проблема при сравнение -0 и +0 равны** -> Отдельная проверка на ноль, без учета последнего bits[3]
  * **Деление** ->
  
  *Процесс деления в столбик можно описать следующей последовательностью действий:*
  ```
a) Берем столько старших разрядов делимого, сколько нужно для того, чтобы сформировать число большее или равное делителю.
b) Определяем первую цифру частного путем подбора, так чтобы произведение делителя и этой цифры было максимально близко к текущему числу, но не превышало его.
c) Вычитаем полученное произведение из текущего числа.
d) Переносим следующий разряд делимого к остатку.
e) Повторяем шаги b-d, пока не используем все разряды делимого.
```
